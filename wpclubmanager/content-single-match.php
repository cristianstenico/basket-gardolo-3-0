...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<?php
		if($played && count($featuredImage) > 0 ) {
				?>
				<div class="hero">
				<!-- data-speed="0.5" data-parallax="scroll" data-image-src="<?php echo $featuredImage[0] ?>"-->
					<div class="hentry-inner">
						<div class="entry-wrapper">
							<header class="entry-header">
								<div class="entry-meta">
									<?php basket_gardolo_3_0_match_meta();  ?>
								</div><!-- .entry-meta -->
								...
								/**
									...
									*/
									if(!$played)
										...
										echo  wpcm_get_team_name(get_post_meta($post->ID, 'wpcm_home_club',true),$post->ID). ' ' . get_post_meta( $post->ID, 'wpcm_home_goals', true ) . ' - ' .
											  wpcm_get_team_name(get_post_meta($post->ID, 'wpcm_away_club',true),$post->ID) . ' ' . get_post_meta( $post->ID, 'wpcm_away_goals', true );
								?>
								</h1>
							</header><!-- .entry-header -->
						</div><!-- .entry-wrapper -->
	...
	<div class="entry-content <?php echo( $played ? 'result' : 'fixture' ); ?>">
